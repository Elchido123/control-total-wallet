(function () {
  ...
  if (!siteKey) {
    alert(
      "Sitio no soportado por Control Total Wallet\nSoportados: " +
        Object.keys(SITES)
          .filter(function (k) {
            return k.split(".").length === 3;
          })
          .map(function (k) {
            return k.charAt(0).toUpperCase() + k.slice(1).split(".")[0];
          })
          .join(", ")
    );
    return;
  ...
  btn.style.cssText =
    ...
    "transition:transform 0.2s ease;font-family:sans-serif;";
  ...
  btn.onclick = async function () {
    ...
    iframe.src =
      "https://control-total-wallet.vercel.app/pay?monto=" +
      monto +
      "&site=" +
    ...
    function handleMessage(e) {
      if (e.origin !== "https://control-total-wallet.vercel.app") return;
      if (e.data.type === "PAYMENT_RESULT") {
        ...
        alert(
          e.data.success
            ? "✅ Pago exitoso - Control Total Wallet"
            : "❌ Pago rechazado - Control Total Wallet"
        );
      }
    }
