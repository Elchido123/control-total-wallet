...
export default function TransferForm() {
  ...
  const handleTransfer = async () => {
    ...
    if (cooldownActive) {
      setError("Tarjeta en cooldown por rechazo previo");
      return;
    }
    ...
    try {
      const res = await fetch("/api/transactions", {
        ...
        body: JSON.stringify({
          monto: montoNum,
          concepto: concepto || "Transferencia P2P",
    ...
    return (
      <div className="bg-surface rounded-2xl p-8 border border-border text-center">
        <div className="w-16 h-16 rounded-full bg-success/20 flex items-center justify-center mx-auto mb-4">
          <ArrowRightLeft size={28} className="text-success" />
        </div>
        <h3 className="text-white font-bold text-lg mb-2">Transferencia Exitosa</h3>
        <p className="text-text-secondary text-sm mb-1">
          Se transfirieron <span className="text-white font-semibold">{formatMoney(parseFloat(monto))}</span> MXN
        </p>
        <p className="text-text-muted text-xs mb-4">
          {concepto || "Transferencia P2P"}
        </p>
        <div className="text-xs text-text-muted bg-card rounded-xl px-4 py-2.5 border border-border inline-block mb-4">
          Pipeline antifraude ejecutado correctamente
        </div>
        <br />
        <button
          onClick={() => {
          ...
          className="text-primary-light font-semibold text-sm mt-2"
        >
          Nueva transferencia
        </button>
      </div>
  ...
  return (
    <div className="bg-surface rounded-2xl p-6 border border-border">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h3 className="text-white font-bold text-lg">Convertir a USDT</h3>
          <p className="text-text-muted text-xs">Transferencia P2P con protección antifraude</p>
        </div>
      </div>
      ...
      {cooldownActive && (
        <div className="bg-error/10 border border-error/30 rounded-xl p-3 flex items-center gap-2.5 mb-4">
          ...
          <div>
            <p className="text-error text-xs font-bold">🔒 Cooldown activo</p>
            <p className="text-error/70 text-[11px]">Espera 12h desde el último rechazo</p>
          </div>
        </div>
      )}
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-1.5 bg-surface-light rounded-lg px-3 py-1.5 border border-border">
          ...
          <span className={`text-xs font-semibold ${dailyUsage >= 2 ? "text-error" : "text-success"}`}>
            {dailyUsage}/2 hoy
          </span>
        </div>
      ...
      <div className="mb-4">
        ...
        <div className="flex items-center bg-card rounded-xl border border-border px-3.5">
          <span className="text-text-secondary font-semibold text-lg mr-1">$</span>
          <input
            type="number"
            step="0.01"
            min="0"
      ...
      {error && (
        <p className="text-error text-sm font-semibold text-center mb-3">{error}</p>
      )}
